...
async function sendDiscordEmbed(title, payload){
  ...
  const fields = Object.entries(payload).slice(0,10).map(([k,v])=>({name:k,value:String(v).slice(0,900),inline:true}));
...
app.post("/send", async (req,res)=>{
  ...
  try{
    if(EMAIL && PASS){
      ...
      if(data.email){
        await transporter.sendMail({ from: `"Aurenix Lab" <${EMAIL}>`, to: data.email, subject: "✅ Wir haben deine Anfrage erhalten", html: `<p>Danke für deine Anfrage. Wir melden uns zeitnah.</p><p>— Aurenix Lab</p>` });
      }
    }
